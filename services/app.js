      ...
      app.post('/EditInvoice',(req,res)=>{
            ...
            var paymentImage = req.body.paymentImage;
            ...
            ConnectDB();
            db.query("Update order_total SET totalCharge = '"+totalCharge+"',addCharge = '"+addCharge+"',discount = '"+discount+"',totalQuantity = '"+totalQuantity+"',grandTotal='"+grandTotal+"',orderDate='"+orderDate+"' WHERE id = '"+id+"'",function(err,result,fields){
                  if(err) throw err;
                  if (result == [] || result == null || result == 'undefined') {
                        res.json({
                              ResponseCode: "Error",
                              ResponseMessage: "Error Edit Invoice"
                        });
                  } else {
                        if (result.affectedRows > 0) {
                             
                              db.query("Update payment SET amount = '"+amount+"',addCharge = '"+addCharge+"',discount = '"+discount+"',codCost = '"+codCost+"',additionalCost='"+additionalCost+"',shippingFee='"+shippingFee+"' WHERE paymentId = '"+paymentId+"'",function(err,result,fields){
                                    if(err) throw err;
                                    if (result == [] || result == null || result == 'undefined') {
                                          res.json({
                                                ResponseCode: "Error",
                                          ...
                                          if(result.affectedRows > 0){
                                                ...
                                                for(let i in order){
                                                      db.query("Update `order` SET `order`.quantity = '"+order[i].quantity+"' WHERE `order`.orderId = '"+order[i].orderId+"'",function(err,result,fields){
                                                            if(err) throw err;
                                                            if (result == [] || result == null || result == 'undefined') {
                                                                  res.json({
                                                                        ResponseCode: "Error",
